...
/**
 * Test class for PersistentDataStore. The PersistentDataStore class relies on
 * DatastoreService, which in turn relies on being deployed in an AppEngine
 * context. Since this test doesn't run in AppEngine, we use
 * LocalServiceTestHelper to do all of the AppEngine setup so we can test. More
 * info:
...
public class PersistentDataStoreTest {
	...
	private final LocalServiceTestHelper appEngineTestHelper = new LocalServiceTestHelper(
	...
	public void testSaveAndLoadActivities() throws PersistentDataStoreException {
		...
		Activity inputActivityOne = new Activity(idOne, idOwner, action1, true, creationOne, thumbnail1);
		...
		Activity inputActivityTwo = new Activity(idTwo, idOwnerTwo, action2, true, creationTwo, thumbnail2);
